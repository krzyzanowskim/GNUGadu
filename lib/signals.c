/* $Id: signals.c,v 1.19 2004/01/28 23:39:28 shaster Exp $ */
...
void hook_signal(GGaduSigID q_name,
		 void (*hook) (GGaduSignal * signal, void (*perl_handler) (GGaduSignal *, gchar *, void *)))
{
 ...
 */
void *signal_emit_full(gpointer src_name, gpointer name, gpointer data, gpointer dest_name,
		       void (*signal_free) (gpointer))
{
	GQuark q_name;
	...
	if (!(q_name = g_quark_try_string(name)))
		q_name = g_quark_from_string(name);

	if (config->all_plugins_loaded && (signalinfo = find_signal((gpointer) q_name)) == NULL)
	{
		print_debug("!!! core : There is no registered signal like : %s \n", name);
		return NULL;
	}
	...
	print_debug("%s : signal_emit %d %s", src_name, q_name, name);
...
void signal_emit_from_thread_full(gpointer src_name, gpointer name, gpointer data, gpointer dest_name,
				  void (*signal_free) (gpointer))
{
	GQuark q_name;
	GGaduSignal *tmpsignal = NULL;
	...
	print_debug("%s : signal_emit_from_thread %d %s", src_name, q_name, name);
...
gboolean signal_from_thread_enabled()
{
	...
	thread_signal_watch =
		g_io_add_watch(thread_signal_channel, G_IO_IN | G_IO_ERR | G_IO_HUP, thread_signal_test_chan, NULL);
