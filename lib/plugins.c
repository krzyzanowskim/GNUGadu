...
void unload_plugin (gchar * name)
{
    ...
    /* najpierw dajemy czas pluginowi na zrobienie porządku ze sobą */
    plugin_handler->destroy_plugin ();
    dlclose (plugin_handler->plugin_so_handler);	/* tego już nie potrzebujemy */

    /* jako, że już nam praktycznie wszystko jedno w jakiej kolejności,
     * to dla porządku czyścimy w takiej kolejności, jak w gg-types.h,
     * żeby o niczym nie zapomnieć
     */

    /* ważne - nie tykać listy dostępnych modułów */
    /* wypierdzielamy plugin z listy włączonych pluginów */
    config->plugins = g_slist_remove (config->plugins, plugin_handler);

    /* nie tykać name */
//    g_free (plugin_handler->name);         /* na pierwszy ogień idzie name */
    g_free (plugin_handler->description);	/* description pod ścianę */

    /* plugin_so_handler załatwiony wcześniej, więc skipujemy */
...
//                      g_free (sig_tmp->name);
    ...
    /* dobry protokół, to zwolniony protokół ;> */
