...
int remote_sig_gui_warn  (char *arg)
{
  signal_emit_full (GGadu_PLUGIN_NAME, "gui show warning",
      g_strdup (arg == NULL ? "<null>" : arg), "main-gui", NULL);
  return 0;
}

int remote_sig_gui_msg  (char *arg)
{
  signal_emit_full (GGadu_PLUGIN_NAME, "gui show message",
      g_strdup (arg == NULL ? "<null>" : arg), "main-gui", NULL);
  return 0;
}
...
int remote_sig_gui_notify (char *arg)
{
  signal_emit (GGadu_PLUGIN_NAME, "gui show notify",
      g_strdup (arg == NULL ? "<null>" : arg), "main-gui");
  return 0;
}
...
int remote_sig_remote_get_icon (char *arg)
{
  ...
  if (!remote_data_icon_directory || !remote_data_icon_filename ||
      !remote_data_icon_tooltip)
  {
...
#endif
  iconsdir = g_build_filename(PACKAGE_DATA_DIR, "pixmaps", "icons",
      remote_data_icon_directory, NULL);
  dir = g_slist_prepend(dir, iconsdir);
  
  while (dir) 
  {
    found_filename = check_file_exists((gchar *) dir->data,
	remote_data_icon_filename);
...
int remote_init (void)
{
  ...
  sck_watch = g_io_add_watch (sck_ch, G_IO_IN | G_IO_ERR | G_IO_HUP,
      remote_recv, NULL);
