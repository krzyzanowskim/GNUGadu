/* $Id: gadu_gadu_plugin.c,v 1.251 2006/11/04 18:28:59 krzyzak Exp $ */
...
static gpointer user_preferences_action(gpointer user_data)
{
...
//                      ggadu_gadu_gadu_reconnect();
	...
	case GG_EVENT_MSG:
		...
		if ((e->event.msg.msgclass & GG_CLASS_CTCP))
		{
			...
			gint port = 0;
			guint32 ip = 0;
			gchar *id = g_strdup_printf("%d", e->event.msg.sender);
			...
			dcc = gg_dcc_get_file(ip, port, (int) ggadu_config_var_get(handler, "uin"), e->event.msg.sender);

			tmp = g_strdup_printf(_("%d sending you a  file"), e->event.msg.sender);
//                      tmp = g_strdup_printf(_("%s (%d) wants to send You a file: %s (%d bytes)"), k->nick, dcc->peer_uin, dcc->file_info.filename, dcc->file_info.size);
			dialog = ggadu_dialog_new_full(GGADU_DIALOG_GENERIC, tmp, "get file", dcc);
			signal_emit(GGadu_PLUGIN_NAME, "gui show dialog", dialog, "main-gui");
...
static gpointer user_remove_user_action(gpointer user_data)
{
	GSList *users = (GSList *) user_data;
...
static gpointer user_add_user_action(gpointer user_data)
{
...
static gpointer register_account(gpointer data)
{
...
static gpointer _register_account_action(gpointer user_data)
{
	GGaduDialog *dialog = NULL;
	GIOChannel *ch = NULL;
...
static gpointer register_account_action(gpointer user_data)
{
	g_thread_create(_register_account_action, NULL, FALSE, NULL);
	return NULL;
}

static gpointer search_action(gpointer user_data)
{
	GGaduDialog *dialog = NULL;
...
gboolean test_chan_dcc_get(GIOChannel * source, GIOCondition condition, gpointer data)
{
	...
	if (d->state == GG_STATE_SENDING_FILE_ACK)
	{
		gchar *dest_path = g_strconcat(g_get_home_dir(), G_DIR_SEPARATOR_S, d->file_info.filename, NULL);
...
//	gchar *path = NULL;
	...
	/* todo */
//      ggadu_config_var_add_with_default(handler, "dcc_dir", VAR_STR);
	...
	if (g_getenv("HOME_ETC"))
	{
		...
		gchar *gg_dir  = g_build_filename(g_getenv("HOME_ETC"), "gg", NULL);

		this_configdir = gg2_dir;

		gg2_path = g_build_filename(gg2_dir, "gadu_gadu", NULL);
		if (!g_file_test(gg2_path, G_FILE_TEST_EXISTS))
		{
			gchar *path = g_build_filename(gg_dir, "config", NULL);
		...
		else if (!ggadu_config_read_from_file(handler, gg2_path))
		{
			g_warning(_("Unable to read configuration file for plugin %s"), "gadu-gadu");
		}
		else
	...
	else
	{
		...
		gchar *gg_dir  = g_build_filename(g_get_home_dir(), ".gg", NULL);
...
void start_plugin()
{
	...
	CHANGE_STATUS_DESCR_DIALOG_SIG = register_signal(handler, "change status descr dialog");
...
*/
	...
	if (signal->name == GET_USER_MENU_SIG)
	{
		...
		ggadu_menu_add_submenu(umenu, ggadu_menu_new_item(_("_Personal data"), user_info_user_action, NULL));
...
*/
	...
	if (signal->name == CHANGE_STATUS_DESCR_DIALOG_SIG)
	{
		GGaduDialog *dialog = signal->data;
		...
		if (ggadu_dialog_get_response(dialog) == GGADU_OK && dialog->user_data)
		{
			...
			{
				kv = ggadu_dialog_get_entries(dialog)->data;
		...
		if (ggadu_dialog_get_response(dialog) == GGADU_OK)
		{
			...
			while (tmplist)
			{
				...
				case GGADU_GADU_GADU_CONFIG_REASON:
					print_debug("changing default reason %s", kv->value);
					ggadu_config_var_set(handler, "reason", kv->value);
					break;
