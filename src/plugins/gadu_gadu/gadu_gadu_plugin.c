...
/*		if (e->event.msg.msgclass == GG_CLASS_CHAT) {
	...
	if ((e->event.msg.msgclass == GG_CLASS_CTCP))	/* dcc part */
	{
	    ...
	    d = gg_dcc_get_file(inet_addr(addr_arr[0]), atoi(addr_arr[1]), (gint) ggadu_config_var_get(handler, "uin"), e->event.msg.sender);
...
gpointer user_preferences_action(gpointer user_data)
{
    ...
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_ID, "GG#", VAR_INT, ggadu_config_var_get(handler, "uin"), VAR_FLAG_NONE);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_PASSWORD, _("Password"), VAR_STR, ggadu_config_var_get(handler, "password"),
			   VAR_FLAG_PASSWORD);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_SERVER, _("Server (optional)"), VAR_STR, ggadu_config_var_get(handler, "server"),
			   VAR_FLAG_NONE);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_PROXY, _("Proxy server (optional)\n[user:pass@]host.com[:80]"), VAR_STR,
    ...
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_AUTOCONNECT_STATUS, _("Autoconnect status"), VAR_LIST, statuslist_names,
			   VAR_FLAG_NONE);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_FRIENDS_MASK, _("Available only for friends"), VAR_BOOL,
			   ggadu_config_var_get(handler, "private"), VAR_FLAG_NONE);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_GADU_GADU_CONFIG_DCC, _("Enable DCC"), VAR_BOOL, ggadu_config_var_get(handler, "dcc"), VAR_FLAG_NONE);
...
gchar *userlist_dump(GSList * list)
{
	...
	line = g_strdup_printf("%s;%s;%s;%s;%s;%s;%s\r\n", k->first_name, k->last_name, k->nick, k->nick, k->mobile, k->group, k->id);
