...
gboolean test_chan(GIOChannel * source, GIOCondition condition, gpointer data)
{
	...
	if (ggadu_config_var_get(handler, "sound_app_file") != NULL)
	    signal_emit(GGadu_PLUGIN_NAME, "sound play file", ggadu_config_var_get(handler, "sound_app_file"),
			"sound*");
	...
	if ((e->event.msg.msgclass == GG_CLASS_CTCP))
	{
	    ...
	    d = gg_dcc_get_file(inet_addr(addr_arr[0]), atoi(addr_arr[1]), (gint) ggadu_config_var_get(handler, "uin"),
				e->event.msg.sender);
	...
	if (ggadu_config_var_get(handler, "sound_msg_file") != NULL)
	    signal_emit(GGadu_PLUGIN_NAME, "sound play file", ggadu_config_var_get(handler, "sound_msg_file"),
			"sound*");
    ...
    case GG_EVENT_STATUS:
	{
	    ...
	    ggadu_convert("CP1250", "UTF-8",
			  ((e->type == GG_EVENT_STATUS) ? e->event.status.descr : e->event.status60.descr), desc_utf8);
	    notify = g_new0(GGaduNotify, 1);
	    notify->id =
		g_strdup_printf("%d", (e->type == GG_EVENT_STATUS) ? e->event.status.uin : e->event.status60.uin);
...
GGaduPlugin *initialize_plugin(gpointer conf_ptr)
{
	...
	this_configdir =
	    g_build_filename(g_get_home_dir(), g_getenv("CONFIG_DIR") ? g_getenv("CONFIG_DIR") : g_getenv("HOME_ETC"),
			     "gg", NULL);
    else
...
void my_signal_receive(gpointer name, gpointer signal_ptr)
{
	...
	if (sp->status != GG_STATUS_NOT_AVAIL)
	{
		...
		{
		    signal_emit(GGadu_PLUGIN_NAME, "gui show warning", g_strdup(_("Unable to change status")),
				"main-gui");
	...
	if (connected && msg)
	{
		...
		if (gg_send_message_confer(session, GG_CLASS_CHAT, g_slist_length(msg->recipients), recipients, message)
		    == -1)
		{
	...
	if (session)
	    signal->data_return =
		(gpointer) ((session->status & GG_STATUS_FRIENDS_MASK) ? session->
			    status ^ GG_STATUS_FRIENDS_MASK : session->status);
