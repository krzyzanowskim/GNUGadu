...
static void egg_tray_icon_send_manager_message(EggTrayIcon * icon, long message, Window window, long data1, long data2,
					       long data3)
{
...
{
	egg_tray_icon_send_manager_message(icon, SYSTEM_TRAY_REQUEST_DOCK, icon->manager_window,
...
guint egg_tray_icon_send_message(EggTrayIcon * icon, gint timeout, const gchar * message, gint len)
{
	...
	egg_tray_icon_send_manager_message(icon, SYSTEM_TRAY_BEGIN_MESSAGE, (Window) gtk_plug_get_id(GTK_PLUG(icon)),
					   timeout, len, stamp);
