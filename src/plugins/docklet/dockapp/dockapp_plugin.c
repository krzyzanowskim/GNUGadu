...
*/
    ggadu_dialog_add_entry(&(d->optlist), GGADU_DOCKAPP_CONFIG_VISIBLE, _("Visible"), VAR_BOOL, ggadu_config_var_get(handler, "dockapp_visible"),
			   VAR_FLAG_NONE);

    if (count > 0)
	ggadu_dialog_add_entry(&(d->optlist), GGADU_DOCKAPP_CONFIG_PROTOCOL, _("Protocol"), VAR_LIST, pluginlist_names, VAR_FLAG_NONE);
...
void my_signal_receive(gpointer name, gpointer signal_ptr)
{
	...
	if (d->response == GGADU_OK)
	{
		...
		case GGADU_DOCKAPP_CONFIG_PROTOCOL:
			...
			{
			    if (ggadu_strcasecmp(ggadu_config_var_get(handler, "dockapp_protocol"), iso))
			    {
