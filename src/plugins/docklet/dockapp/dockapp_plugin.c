...
GdkPixbuf *dockapp_create_pixbuf(const gchar * directory,
   const gchar * filename)
{
  ...
  if (directory) {
    iconsdir =
       g_build_filename(PACKAGE_DATA_DIR, "pixmaps", "icons", directory,
       NULL);
...
void redraw_dockapp()
{
  ...
  gdk_draw_pixmap(icon_dockapp->window, dock_gc, launch_pixmap, 0, 0, 0, 0,
     64, 64);
}
...
void draw_pixmap()
{
  ...
  /* draw icons */
  if (icon1_img != NULL)
    gdk_draw_pixbuf(launch_pixmap, dock_gc, icon1_img, 0, 0, icon1.x, icon1.y,
       icon1.width, icon1.height, GDK_RGB_DITHER_NONE, 0, 0);
  /* draw 'new message' icon */
  if ((icon2_img != NULL) && ((blink_no % 2) == 1))
    gdk_draw_pixbuf(launch_pixmap, dock_gc, icon2_img, 0, 0, icon2.x, icon2.y,
       icon2.width, icon2.height, GDK_RGB_DITHER_NONE, 0, 0);

  font =
     gdk_font_load("-misc-fixed-medium-r-normal-*-10-*-*-*-*-*-iso8859-2");
  ...
  for (i = 0; i < NNICK; i++) {
    ...
    gdk_draw_text(launch_pixmap, font, dock_gc, 6, 34 + (i * 10),
       prev_nick[i], strlen(prev_nick[i]));
  }
  gdk_font_unref(font);
}
...
void dockapp_clicked(GtkWidget * widget, GdkEventButton * ev, gpointer data)
{
  ...
  if (ev->button == 1) {
    ...
    /* on icon1 or icon2 ? */
    if (btn_clicked(&icon1, ev->x, ev->y) ||
       btn_clicked(&icon2, ev->x, ev->y)) {
      ...
      draw_pixmap();
      redraw_dockapp();
      signal_emit_full(GGadu_PLUGIN_NAME, "gui show invisible chats", NULL,
         "main-gui", NULL);
    }
  }
}
...
GGaduMenu *build_plugin_menu()
{
  ...
  ggadu_menu_add_submenu(item_gg, ggadu_menu_new_item(_("Preferences"),
        dockapp_preferences_action, NULL));
...
void create_dockapp()
{
  ...
  gtk_window_set_wmclass(GTK_WINDOW(status_dockapp), "GM_Statusdockapp",
     "gg2");
  ...
  gtk_window_set_type_hint(GTK_WINDOW(status_dockapp),
  ...
  gtk_widget_set_events(icon_dockapp,
     GDK_BUTTON_PRESS_MASK | GDK_EXPOSURE_MASK);
  g_signal_connect(G_OBJECT(icon_dockapp), "button-press-event",
     G_CALLBACK(dockapp_clicked), NULL);
  g_signal_connect(G_OBJECT(icon_dockapp), "expose-event",
     G_CALLBACK(redraw_dockapp), NULL);
  ...
  XSetWMHints(GDK_DISPLAY(), GDK_WINDOW_XWINDOW(status_dockapp->window),
     &wmhints);
...
void start_plugin()
{
  ...
  signal_emit(GGadu_PLUGIN_NAME, "gui register menu", dockapp_menu,
     "main-gui");
...
GGaduPlugin *initialize_plugin(gpointer conf_ptr)
{
  ...
  handler =
     (GGaduPlugin *) register_plugin(GGadu_PLUGIN_NAME,
     _("WindowMaker Dockapp"));
  register_signal_receiver((GGaduPlugin *) handler,
...
void destroy_plugin()
{
  ...
  //icon3_img = NULL;
  if (dockapp_menu) {
    signal_emit(GGadu_PLUGIN_NAME, "gui unregister menu", dockapp_menu,
       "main-gui");
