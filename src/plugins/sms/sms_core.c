...
/* laczenie sie z wybranym hostem, zwraca -1 przy błedzie, inaczej 0 */
...
gpointer send_ORANGE_stage2(SMS * message)
{
	...
	if (g_strstr_len(recv_buff, i, "SMS został wysłany"))
		sms_message(message->number, _("SMS has been sent"));

	else if (g_strstr_len(recv_buff, i, "Podano błędne hasło, SMS nie został wysłany"))
	...
	else if (g_strstr_len(recv_buff, i, "nie ma aktywnej usługi"))
...
gint send_PLUS(SMS * message)
{
	...
	if (!strlen(recv_buff))
		ret = ERR_SERVICE;
	else if (g_strstr_len(recv_buff, i, "wiadomość została wysłana na numer"))
		ret = TRUE;
	else if (g_strstr_len(recv_buff, i, "podano zły numer"))
		ret = ERR_BAD_RCPT;
