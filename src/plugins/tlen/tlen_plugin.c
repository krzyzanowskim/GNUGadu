/* $Id: tlen_plugin.c,v 1.46 2004/01/17 00:45:05 shaster Exp $ */
...
*/
	...
	to_utf8("ISO-8859-2", item->firstname, k->first_name);
    if ((item->lastname) != NULL)
	to_utf8("ISO-8859-2", item->lastname, k->last_name);
    if ((item->nick) != NULL)
	to_utf8("ISO-8859-2", item->nick, k->nick);
    if ((item->city) != NULL)
	to_utf8("ISO-8859-2", item->city, k->city);
...
gpointer user_preferences_action(gpointer user_data)
{
    ...
    ggadu_dialog_add_entry(&(d->optlist), GGADU_TLEN_UIN, _("Tlen login"), VAR_STR, ggadu_config_var_get(handler, "login"), VAR_FLAG_NONE);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_TLEN_PASSWORD, _("Password"), VAR_STR, ggadu_config_var_get(handler, "password"), VAR_FLAG_PASSWORD);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_TLEN_LOG, _("Log chats to history file"), VAR_BOOL, ggadu_config_var_get(handler, "log"),
			   VAR_FLAG_NONE);
    ggadu_dialog_add_entry(&(d->optlist), GGADU_TLEN_AUTOCONNECT, _("Autoconnect on startup"), VAR_BOOL, ggadu_config_var_get(handler, "autoconnect"),
			   VAR_FLAG_NONE);
...
void start_plugin()
{
	...
	ggadu_tlen_login(ggadu_config_var_get(handler, "autoconnect_status") ? (gpointer) ggadu_config_var_get(handler, "autoconnect_status")
			 : (gpointer) TLEN_STATUS_AVAILABLE);
}
