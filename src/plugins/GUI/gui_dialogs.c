/* $Id: gui_dialogs.c,v 1.28 2004/01/17 00:44:59 shaster Exp $ */
#ifdef HAVE_CONFIG_H
#  include <config.h>
#endif
...
GtkWidget *gui_build_dialog_gtk_table(GSList * list, gint cols)
{
	...
	case VAR_COLOUR_CHOOSER:
		...
		g_signal_connect_swapped(button_entry, "clicked", G_CALLBACK(gui_dialog_show_colorchooser), txt_entry);
	    }
	    break;
	case VAR_IMG:
	    need_label = FALSE;
...
void gui_user_data_window(gpointer signal, gboolean change)
{
    ...
    adduserwindow =
	gtk_dialog_new_with_buttons((change) ? _("Change User") : _("Add User"), NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CANCEL,
    ...
    g_signal_connect(G_OBJECT(adduserwindow), "response",
		     (change) ? G_CALLBACK(gui_change_user_window_response) : G_CALLBACK(gui_add_user_window_response), signal_cpy(signal));
...
void gui_show_message_box(gint type, gpointer signal)
{
    ...
    warning = gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, type, GTK_BUTTONS_OK, txt);

    gp = gui_find_protocol(((GGaduSignal *) signal)->source_plugin_name, protocols);

    title =
	g_strdup_printf("%s: %s", (gp) ? gp->p->display_name : (char *) ((GGaduSignal *) signal)->source_plugin_name,
...
void gui_show_dialog(gpointer signal, gboolean change)
{
    ...
    dialog =
	gtk_dialog_new_with_buttons(d->title, NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK,
