/* $Id: gui_preferences.c,v 1.46 2004/01/17 00:44:59 shaster Exp $ */
#ifdef HAVE_CONFIG_H
#  include <config.h>
#endif
...
static gboolean save_selected_plugins(GtkTreeModel * model, GtkTreePath * path, GtkTreeIter * iter, gpointer data)
{
	...
	if (!ggadu_strcasecmp(name, "main-gui"))
	{
	    signal_emit_full("main-gui", "gui show warning",
			     g_strdup(_("GUI is selected as DISABLED\nIf you are sure, you have to restart GNU Gadu to take effect")), "main-gui",
			     NULL);
	    return FALSE;
	}
	unload_plugin(name);
...
GtkWidget *gui_plugins_mgr_tab()
{
	...
	gtk_tree_store_set(GTK_TREE_STORE(store), &iter, PLUGINS_MGR_NAME, (gchar *) pl->name, PLUGINS_MGR_ENABLE, tmpvar, -1);
...
void gui_preferences(GtkWidget * widget, gpointer data)
{
    ...
    preferences =
	gtk_dialog_new_with_buttons(_("Preferences"), NULL, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT,
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(entry), (gint) ggadu_config_var_get(gui_handler, "chat_paned_size"));
	...
	ggadu_config_var_set(gui_handler, "blink_interval", (gpointer) gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(blink_interval)));
	...
	ggadu_config_var_set(gui_handler, "hide_on_start", (gpointer) gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(hide_on_start)));

	ggadu_config_var_set(gui_handler, "hide_toolbar", (gpointer) ! gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(hide_toolbar)));

	ggadu_config_var_set(gui_handler, "theme", (gpointer) g_strdup(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(combo_theme)->entry))));
	ggadu_config_var_set(gui_handler, "icons", (gpointer) g_strdup(gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(combo_icons)->entry))));
