/* $Id: gtkanimlabel.c,v 1.15 2004/10/19 10:51:25 krzyzak Exp $ */
...
)
{
	...
	gdk_draw_drawable(widget->window, widget->style->bg_gc[widget->state], anim_label->pixmap, 0, 0, anim_label->pos_x, 0, widget->allocation.width, widget->allocation.height);
...
)
{
	...
	pango_layout_get_extents(anim_label->layout, NULL, &prect);
	anim_label->pixmap = gdk_pixmap_new(widget->window, PANGO_PIXELS(prect.width) + 5, PANGO_PIXELS(prect.height), -1);

	/* czyscze pixmape */
	gdk_draw_rectangle(anim_label->pixmap, widget->style->bg_gc[widget->state], TRUE, 0, 0, PANGO_PIXELS(prect.width) + 5, PANGO_PIXELS(prect.height));
