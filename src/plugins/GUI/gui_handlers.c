...
void handle_show_invisible_chats(GGaduSignal * signal)
{
	...
	gtk_window_move(GTK_WINDOW(window), (gint) ggadu_config_var_get(gui_handler, "left"), (gint) ggadu_config_var_get(gui_handler, "top"));
...
*/
	...
	gtk_tree_store_set(users_treestore, &parent_iter, 1, g_strdup_printf("%s (%d/%d)", gp->p->display_name, 0, g_slist_length(gp->userlist)), -1);
    }
...
void auto_away_start(gui_protocol * gp)
{
    ...
    {
	gp->aaway_timer =
	    g_timeout_add(ggadu_config_var_get(gui_handler, "auto_away_interval") ? ((gint) ggadu_config_var_get(gui_handler, "auto_away_interval")) *
