...
gboolean nick_list_clicked(GtkWidget *widget, GdkEventButton *event, gpointer user_data) 
{
    ...
    gtk_tree_selection_selected_foreach( gtk_tree_view_get_selection(GTK_TREE_VIEW(widget)),
...
//			case 0: ip = g_strdup_printf("\n[brak p2p]");break;
    ...
    if ((event->type == GDK_BUTTON_PRESS) && (event->button == 3))
    {
	    ...
	    if ((gp) && (gp->userlist_menu)) {
		umenu = gp->userlist_menu;
...
void gui_msg_receive(GGaduSignal *signal)
{
	...
	if (gp != NULL) {
		...
		if (msg->message == NULL) showwindow = TRUE;
		...
		if (!session->chat) 
		{
		    ...
		    if ((gint)config_var_get(gui_handler,"use_xosd_for_new_msgs") == TRUE &&
