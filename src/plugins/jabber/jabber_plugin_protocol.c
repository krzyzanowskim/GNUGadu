...
{
/*    if (jabber_session->state != NET_OFF) 
	{
	jabber_presence(jabber_session,JABBER_STATUS_UNAVAILABLE,""); 
...
*/
	...
	{
	 print_debug(_("Something went bad witn iksemel!\n"));
//	 exit(1);
	}
    }
...
/*    session = tlen_init();
    tlen_set_auth(session,config_var_get(handler, "login"), config_var_get(handler, "password"));
