...
{                                                                               
	...
	x = iks_make_pres(IKS_TYPE_UNSUBSCRIBE, 0, k->id, NULL);
	iks_send(jabber_session->parser, x);
	iks_delete(x);
...
gboolean updatewatch(struct netdata *sess)
{
	...
	{
	    if (
	    g_source_remove(tag) == TRUE)
	    {
		g_io_channel_unref(source_chan);
	    } else return FALSE;
	}
	if ((source_chan = g_io_channel_unix_new(iks_fd(sess->parser))) == NULL) return FALSE;
	{
...
GSList *status_init() 
{
    ...
    sp = g_new0(GGaduStatusPrototype, 7);
...
void start_plugin()
{
	...
	g_thread_create(login,(gpointer) JABBER_STATUS_AVAILABLE,FALSE,NULL);
    }
}
...
GGaduPlugin *initialize_plugin(gpointer conf_ptr) 
{
    ...
    set_config_file_name((GGaduPlugin *)jabber_handler, g_build_filename(this_configdir,"jabber",NULL));
